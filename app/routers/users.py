...
@router.get("/me", response_model=User, dependencies=[Depends(is_valid_user)])
...
@router.put("/me", response_model=User, dependencies=[Depends(is_valid_user)])
def update_user_me(
    *,
...
@router.post("/update", response_model=User, dependencies=[Depends(is_valid_admin)])
...
@router.post("/newpw", response_model=User, dependencies=[Depends(is_valid_user)])
def new_pw(
    *,
    db: Session = Depends(get_db),
...
@router.post("/users", response_model=User, dependencies=[Depends(is_valid_admin)])
...
@router.get("/users", response_model=List[UserID], dependencies=[Depends(is_valid_admin)])
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.get_users(db, skip=skip, limit=limit)
    return users


@router.get("/users/{user_id}", response_model=User, dependencies=[Depends(is_valid_admin)])
...
@router.delete("/delete/{id}", response_model=User, dependencies=[Depends(is_valid_admin)])
def delete_item(
    *,
    db: Session = Depends(get_db),
    id: int,
